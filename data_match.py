...
def main():
    ...
    for day in range(DAYS):
        if DAYS % (day + 1) == 0:
            split_day.append(DAYS // (day + 1))
    columns = ['dates', 'SEED', 'Num of Pairs', '未ペア数（男）', '未ペア数（女）', '総数（男）', '総数（女）']
    s_df = pd.DataFrame(columns=columns)
    m_df = pd.DataFrame(columns=columns)
    row_num = 0
    for i in range(10):
        random.seed(i)
        ...
        for day in range(DAYS):
            ...
            if DAYS % (day + 1) == 0:
                ...
                print("■ペア数")
                print(len(s))
                s_record.append(DAYS // (day + 1))
                s_record.append(i)
                s_record.append(len(s))
                s_record.append(len(l_men) - len(s))
                s_record.append(len(l_women) - len(s))
                s_record.append(len(l_men))
                s_record.append(len(l_women))
                print("■不満足度")
                ...
                print("■ペア数")
                print(len(m))
                m_record.append(DAYS // (day + 1))
                m_record.append(i)
                m_record.append(len(m))
                m_record.append(len(l_men) - len(m))
                m_record.append(len(l_women) - len(m))
                m_record.append(len(l_men))
                m_record.append(len(l_women))
                print("■不満足度")
                dissatisfaction_score = calc_dissatisfaction(m, l_men, l_women)
                print("見かけ", apparent_m_dissatisfaction)
                print("実際", dissatisfaction_score)
                s_df.loc[row_num] = s_record
                m_df.loc[row_num] = m_record
                row_num += 1
    s_pair_df = pd.crosstab(s_df['dates'], s_df['SEED'], values=s_df['Num of Pairs'], aggfunc='sum')
    s_pair_df.plot(grid=True, title='S', kind='line')
    plt.ylabel('Num of Pairs')
    s_pair_df = pd.concat([s_pair_df.T, s_pair_df.T.describe()])
    s_unmatched_men_df = pd.crosstab(s_df['SEED'], s_df['dates'], values=s_df['未ペア数（男）'], aggfunc='sum')
    s_unmatched_men_df = pd.concat([s_unmatched_men_df, s_unmatched_men_df.describe()])
    s_unmatched_women_df = pd.crosstab(s_df['SEED'], s_df['dates'], values=s_df['未ペア数（女）'], aggfunc='sum')
    s_unmatched_women_df = pd.concat([s_unmatched_women_df, s_unmatched_women_df.describe()])

    m_pair_df = pd.crosstab(m_df['dates'], m_df['SEED'], values=m_df['Num of Pairs'], aggfunc='sum')
    m_pair_df.plot(grid=True, title='M', kind='line')
    plt.ylabel('Num of Pairs')
    m_pair_df = pd.concat([m_pair_df.T, m_pair_df.T.describe()])
    m_unmatched_men_df = pd.crosstab(m_df['SEED'], m_df['dates'], values=m_df['未ペア数（男）'], aggfunc='sum')
    m_unmatched_men_df = pd.concat([m_unmatched_men_df, s_unmatched_men_df.describe()])
    m_unmatched_women_df = pd.crosstab(m_df['SEED'], m_df['dates'], values=m_df['未ペア数（女）'], aggfunc='sum')
    m_unmatched_women_df = pd.concat([m_unmatched_women_df, m_unmatched_women_df.describe()])
    with pd.ExcelWriter('output.xlsx') as writer:
        s_pair_df.to_excel(writer, sheet_name='S_ペア数')
        s_unmatched_men_df.to_excel(writer, sheet_name='S_未ペア数（男）')
        s_unmatched_women_df.to_excel(writer, sheet_name='S_未ペア数（女）')
        m_pair_df.to_excel(writer, sheet_name='M_ペア数')
        m_unmatched_men_df.to_excel(writer, sheet_name='M_未ペア数（男）')
        m_unmatched_women_df.to_excel(writer, sheet_name='M_未ペア数（女）')
