...
def main():
    # fig, (axS, axM) = plt.subplots(ncols=2, figsize=(16, 6), sharex=True)
    # columns = []
    # for day in range(DAYS):
    #     if DAYS % (day + 1) == 0:
    #         columns.append(str(int(DAYS / (day + 1))))
    columns = range(10)
    index_list = ['シード値', '日数', 'ペア数', '未ペア数（男）', '未ペア数（女）', '総数（男）', '総数（女）', 'index名']
    # s_df = pd.DataFrame(columns=columns, index=index_list)
    # m_df = pd.DataFrame(columns=columns, index=index_list)
    s_df = pd.DataFrame(columns=index_list)
    seed_num = 10
    # s_data = [[], [], [], [], [], [], [], [], [], [], []]
    # m_data = [[], [], [], [], [], [], [], [], [], [], []]
    row_num = 0
    for i in range(seed_num):
        ...
        for day in range(DAYS):
            ...
            if DAYS % (day + 1) == 0:
                ...
                print(len(s))
                s_data.append(int(i))
                s_data.append(int(DAYS / (day + 1)))
                s_data.append(len(s))
                s_data.append(len(l_men)-len(s))
                s_data.append(len(l_women)-len(s))
                s_data.append(len(l_men))
                s_data.append(len(l_women))
                s_data.append('ペア数')
                s_df.loc[str(row_num)] = s_data
                # s_df[str(int(DAYS / (day + 1)))] = s_data
                # plot_s_list.append(len(s))
                print("■ペアになれなかった人")
                print(list(set(list(l_men.keys()) + list(l_women.keys())) - set(
                    [flatten for inner in s for flatten in inner])))
                print("■不満足度")
                ...
                print("■ペア数")
                print(len(m))
                # m_data[count].append(len(m))
                # m_df[str(int(DAYS / (day + 1)))] = m_data
                ...
                count += 1
                row_num += 1
    # m_df = pd.DataFrame(m_data, columns=columns)
    print(s_df)
    # print(pd.crosstab([s_df['ペア数']], [s_df['日数'], s_df['シード値']]))
    df = pd.pivot_table(s_df, index='index名', columns=['日数', 'シード値'], values='ペア数', aggfunc='sum')
    # print(pd.pivot_table(s_df, index='index名', columns=['日数', 'シード値'], values='ペア数', aggfunc='sum'))
    # print(m_df)
    # fig, axes = plt.subplots(2, 2)
    # s_df.plot(ax=axes[0][0])
    # axes[0][0].set_title('S')
    # axes[0][0].grid()
    # m_df.plot(ax=axes[0][1])
